...
from utility import HTTPMethodEnum, make_api_request
from config import sentry_token, logger
from typing import Optional, Any


def parse_link_header(link_header: str) -> list[dict[str, str]]:
    parsed_links = list()
...
def get_issues(settings_fields: list[dict[str, Any]], offset: Optional[int]=None) -> \
   tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    if not org_slug_field or not proj_slug_field:
        logger.warning(f"[SENTRY] Fields 'orgSlug' or 'projSlug' are not present")
        return list()
