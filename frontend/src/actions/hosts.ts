...
export async function UpdateHost({ uuid, body }: {uuid: string, body: { hostname: string, os: string, ip4?: string, ip6?: string, teamID: string }}): Promise<boolean> {
    const response = await fetch(`/api/hosts/${uuid}`, {
        method: "PUT",
        headers: {
            "Content-Type": "application/json"
...
export async function DeleteHost(uuid: string): Promise<boolean> {
    const response = await fetch(`/api/hosts/${uuid}`, {
        method: "DELETE"
    });
    if (!response.ok) {
